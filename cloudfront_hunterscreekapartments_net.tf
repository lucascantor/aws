...
# ------------------------------------------------------------------------------------------
# mta-sts.hunterscreekapartments.net

resource "aws_cloudfront_distribution" "mta_sts_hunterscreekapartments_net" {
  aliases = [
    "mta-sts.hunterscreekapartments.net",
  ]
  default_root_object = "index.html"
  default_cache_behavior {
    ...
    target_origin_id           = "S3-mta-sts.hunterscreekapartments.net"
    viewer_protocol_policy     = "redirect-to-https"
  }
  enabled         = true
  is_ipv6_enabled = true
  origin {
    domain_name = "mta-sts.hunterscreekapartments.net.s3.amazonaws.com"
    origin_id   = "S3-mta-sts.hunterscreekapartments.net"
    s3_origin_config {
      origin_access_identity = aws_cloudfront_origin_access_identity.identities["mta-sts.hunterscreekapartments.net"].cloudfront_access_identity_path
    }
  }
  restrictions {
    geo_restriction {
