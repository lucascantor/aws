...
resource "aws_route53_record" "kindredcode_net__A" {
  ...
  alias {
    name                   = var.cloudfront_distribution_domain_names["kindredcode.com"]
...
resource "aws_route53_record" "dmarc_kindredcode_net__TXT" {
  ...
  ttl     = "3600"
  records = [
    "v=DMARC1;p=reject;sp=reject;adkim=s;aspf=s;fo=1;rua=mailto:dmarc@kindredcode.net,mailto:dmarc@kindredcode.net",
  ]
}
